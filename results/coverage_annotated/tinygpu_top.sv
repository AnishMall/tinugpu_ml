...
%000004       mem_rdata_stage_q  <= '0;
%000004       mem_rvalid_stage_q <= 1'b0;
%000004       mem_cmd_valid_q    <= 1'b0;
%000004       mem_cmd_we_q       <= 1'b0;
%000004       mem_cmd_addr_q     <= '0;
%000004       mem_cmd_wdata_q    <= '0;
%000004       mem_cmd_wstrb_q    <= '0;
%000004       mem_read_pending_q <= 1'b0;
 1056276     end else begin
 1056276       mem_rvalid_stage_q <= 1'b0;
        
~975893       if (mem_stage_ready && mem_req_raw) begin
~080383         mem_cmd_valid_q <= 1'b1;
~080383         mem_cmd_we_q    <= mem_we_raw;
~080383         mem_cmd_addr_q  <= mem_addr_raw;
~080383         mem_cmd_wdata_q <= mem_wdata_raw;
~080383         mem_cmd_wstrb_q <= mem_wstrb_raw;
              end
        
~975893       if (mem_cmd_valid_q && mem_ready) begin
~080383         mem_cmd_valid_q <= 1'b0;
~069157         if (!mem_cmd_we_q) begin
~069157           if (mem_rvalid) begin
~069157             mem_read_pending_q <= 1'b0;
~069157             mem_rdata_stage_q  <= mem_rdata;
~069157             mem_rvalid_stage_q <= 1'b1;
~002202           end else begin
~002202             mem_read_pending_q <= 1'b1;
                  end
                end
              end
        
~1056276       if (mem_read_pending_q && mem_rvalid) begin
~002202         mem_read_pending_q <= 1'b0;
~002202         mem_rdata_stage_q  <= mem_rdata;
~002202         mem_rvalid_stage_q <= 1'b1;
              end
            end
          end
