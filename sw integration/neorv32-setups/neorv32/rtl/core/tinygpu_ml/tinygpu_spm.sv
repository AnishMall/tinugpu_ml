module tinygpu_spm import tinygpu_pkg::*; #(
  ...
  function automatic logic [7:0] read_byte_region(
    ...
    begin
      ...
      case (region)
        REGION_A: begin
          if (addr < A_BYTES)
            read_byte_region = a_mem[addr];
        end
        REGION_B: begin
          if (addr < B_BYTES)
            read_byte_region = b_mem[addr];
        end
        REGION_C: begin
          if (addr < C_BYTES)
            read_byte_region = c_mem[addr];
        end
        default: begin
          read_byte_region = '0;
        end
  ...
  task automatic write_byte_region(
    input logic [1:0] region,
    input int unsigned addr,
