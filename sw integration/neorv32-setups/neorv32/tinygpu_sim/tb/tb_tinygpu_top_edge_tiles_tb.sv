module tb_tinygpu_top_edge_tiles_tb;
  ...
  initial begin
    ...
    for (int r = 0; r < M_DIM; r++) begin
      for (int k = 0; k < K_DIM; k++) begin
        mem_bytes[A_BASE + (r * K_DIM) + k] = 8'(1 + (r * K_DIM) + k);
      end
    end

    for (int k = 0; k < K_DIM; k++) begin
      for (int c = 0; c < N_DIM; c++) begin
        mem_bytes[B_BASE + (k * N_DIM) + c] = 8'(2 + (k * N_DIM) + c);
      end
    end
    ...
    for (int r = 0; r < M_DIM; r++) begin
      for (int c = 0; c < N_DIM; c++) begin
        if (read_word(C_BASE + ((r * N_DIM) + c) * 4) !== expected_c(r, c)) begin
          $fatal(
            1,
            "edge tile mismatch at (%0d,%0d): got %0d expected %0d",
            r,
            c,
    ...
    $display("tb_tinygpu_top_edge_tiles_tb PASS");
    $finish;
  end
