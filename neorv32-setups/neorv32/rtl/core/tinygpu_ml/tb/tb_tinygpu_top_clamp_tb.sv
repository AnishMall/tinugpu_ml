module tb_tinygpu_top_clamp_tb;
  ...
  initial begin
    ...
    // Produces +200 and -200 before clamp.
    ...
    mmio_wr(REG_FLAGS,     FLAG_CLAMP_EN | FLAG_DST_INT32);
    ...
    if (!status_word[1]) $fatal(1, "done sticky was not set");
    if (status_word[0])  $fatal(1, "busy should be low after completion");
    if (read_word(C_BASE + 0)   !== 32'sd127)  $fatal(1, "C[0][0] clamp mismatch");
    if (read_word(C_BASE + 4)   !== 32'sd127)  $fatal(1, "C[0][1] clamp mismatch");
    if (read_word(C_BASE + 8)   !== -32'sd128) $fatal(1, "C[1][0] clamp mismatch");
    if (read_word(C_BASE + 12)  !== -32'sd128) $fatal(1, "C[1][1] clamp mismatch");

    $display("tb_tinygpu_top_clamp_tb PASS");
    $finish;
  end
